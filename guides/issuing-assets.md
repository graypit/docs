...
One of DigitalBits’s most powerful features is the ability to trade any kind of asset, Loyalty Points & Rewards, special coupons, US dollars, bitcoins, or just about anything you like.

This works in DigitalBits because an asset is really just a credit from a particular account. When you trade US dollars on the DigitalBits network, you don’t actually trade US dollars—you trade US dollars *credited from a particular account.* That account could be a bank, but if your neighbor had a banana plant, they might issue banana assets that you could trade with other people.

Every asset type (except digitalbits) is defined by two properties:
...
In the DigitalBits SDK, assets are represented with the `Asset` class:

<code-example name="Representing Assets">

```js
var astroDollar = new DigitalBitsSdk.Asset(
  'AstroDollar', 'GC2BKLYOOYPDEFJKLKY6FNNRQMGFLVHJKQRGNSSRRGSMPGF32LHCQVGF');
```

```java
KeyPair issuer = DigitalBitsSdk.Keypair.fromAccountId(
  "GC2BKLYOOYPDEFJKLKY6FNNRQMGFLVHJKQRGNSSRRGSMPGF32LHCQVGF");
Asset astroDollar = Asset.createNonNativeAsset("AstroDollar", issuer);
```

```json
// Wherever assets are used in Frontier, they use the following JSON structure:
{
...
However, other people can’t receive your asset until they’ve chosen to trust it. Because a DigitalBits asset is really a credit, you should trust that the issuer can redeem that credit if necessary later on. You might not want to trust your neighbor to issue banana assets if they don’t even have a banana plant, for example.

An account can create a *trustline,* or a declaration that it trusts a particular asset, using the [change trust operation](concepts/list-of-operations.md#change-trust). A trustline can also be limited to a particular amount. If your banana-growing neighbor only has a few plants, you might not want to trust them for more than about 200 bananas. *Note: each trustline increases an account’s minimum balance by 0.5 digitalbits (the base reserve). For more details, see the [fees guide](concepts/fees.html#minimum-balance).*
...
Sending and receiving custom assets is very similar to [sending and receiving digitalbits](get-started/transactions.md#building-a-transaction). Here’s a simple example:
...
var issuingKeys = DigitalBitsSdk.Keypair
  .fromSecret('SCZANGBA5YHTNYVVV4C3U252E2B6P6F5T3U6MM63WBSBZATAQI3EBTQ4');
var receivingKeys = DigitalBitsSdk.Keypair
  .fromSecret('SDSAVCRE5JRAI7UFAVLE5IMIZRD6N6WOJUWKY4GFN34LOBEEUS4W2T2D');

// Create an object to represent the new asset
var astroDollar = new DigitalBitsSdk.Asset('AstroDollar', issuingKeys.publicKey());

// First, the receiving account must trust the asset
server.loadAccount(receivingKeys.publicKey())
  .then(function(receiver) {
    var transaction = new DigitalBitsSdk.TransactionBuilder(receiver)
      // The `changeTrust` operation creates (or alters) a trustline
      // The `limit` parameter below is optional
      .addOperation(DigitalBitsSdk.Operation.changeTrust({
        asset: astroDollar,
        limit: '1000'
      }))
      .build();
  ...
  .then(function(issuer) {
    var transaction = new DigitalBitsSdk.TransactionBuilder(issuer)
...
To do this you must do two simple things. First, add a section in your [digitalbits.toml file](concepts/digitalbits-toml.html) that contains the necessary meta fields:
```
# digitalbits.toml example asset
[[CURRENCIES]]
code="GOAT"
...
Second, use the [set options operation](https://developer.digitalbits.io/guides/concepts/list-of-operations.html#set-options) to set the `home_domain` of your issuing account to the domain where the above digitalbits.toml file is hosted. The following code sets the home domain:
...
In the simplest situations, you can issue assets from your everyday DigitalBits account. However, if you operate a financial institution or a business, you should keep a separate account specifically for issuing assets. Why?
...
- Easier tracking: because an asset represents a credit, it disappears when it is sent back to the account that issued it. To better track and control the amount of your asset in circulation, pay a fixed amount of the asset from the issuing account to the working account that you use for normal transactions.
  ...
  The issuing account can issue the asset when more of the underlying value (like actual bananas or dollar bills) is on hand and the accounts involved in public transactions never have to worry about how much is available outside DigitalBits.

- Keeping trust simple: as your usage of DigitalBits grows, you might consider having multiple accounts for a variety of reasons, such as making transactions at high rates. Keeping a canonical issuing account makes it easier for others to know which account to trust.
...
```js
DigitalBitsSdk.Network.useTestNetwork();
var flags = DigitalBitsSdk.xdr.AccountFlags;
var transaction = new DigitalBitsSdk.TransactionBuilder(issuingAccount)
  .addOperation(DigitalBitsSdk.Operation.setOptions({
    setFlags: DigitalBitsSdk.AuthRevocableFlag | DigitalBitsSdk.AuthRequiredFlag
  }))
  .build();
transaction.sign(issuingKeys);
...
```java
import io.digitalbits.sdk.AccountFlag;
