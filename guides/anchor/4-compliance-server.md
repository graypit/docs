...
![](assets/Compliance-Server-1-v2.png)
...
url = "dbuser:dbpassword@/digitalbits_compliance"
...
```toml
port = 8001
horizon = "https://frontier.testnet.digitalbits.io"
...
- `fetch_info` is sent a federation address (like `tunde_adebayo*your_org.com`) and should return all the information necessary for another organization to perform compliance checks. It can be any data you deem reasonable and must be formatted as JSON.
    ...
    app.post('/compliance/fetch_info', function (request, response) {
      ...
      // up a customer by their DigitalBits account and return account information.
    ...
    public Response fetchInfo(
      ...
      // find customers by their DigitalBits account and return account information.
      try {
...
When other organizations need to contact your compliance server to authorize a payment to one of your customers, they consult your domain’s `digitalbits.toml` file for the address, just as when finding your federation server.

For compliance operations, you’ll need to list two new properties in your `digitalbits.toml`:
